#!/usr/bin/env bash
# Robust CNN+LSTM encoder: deep_only and hybrid experiments (fromscratch + frompretrain ep20/50/100).
set -euo pipefail
source "$(dirname "$0")/_common.sh"
export BATCH_SIZE="${BATCH_SIZE_LARGE:-512}"

# Pretrain once; returns output_dir containing ep20/ep50/ep100 snapshots.
TS2VEC_DIR=$(run_ts2vec_pretrain "robust")

for MODE in deep_only hybrid; do
    # From scratch (default initialisation)
    RUN_SUFFIX=fromscratch run_experiment "robust" "$MODE"
    RUN_SUFFIX=fromscratch run_finetune   "robust" "$MODE"
    ...
    for EP in 20 50 100; do
        ...
        RUN_SUFFIX="frompretrain_ep${EP}" run_experiment "robust" "$MODE" \
            --init_encoder_from "$CKPT"
        RUN_SUFFIX="frompretrain_ep${EP}" run_finetune   "robust" "$MODE"
    done
done
