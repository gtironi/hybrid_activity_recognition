...
#   - CNN-LSTM  × {deep_only, hybrid} × {fromscratch, frompretrain_raw_ep100} (+ finetune)
#   - Robust    × {deep_only, hybrid} × {fromscratch, frompretrain_raw_ep100} (+ finetune)
...
# (5 s / 50 % overlap = 125 samples) and is shared across the 3 feature datasets.
...
# raw windowed parquet path / DATASET_ID encode 5 s @ 25 Hz windows).
# ---------------------------------------------------------------------------
export WINDOW_LEN="${WINDOW_LEN:-125}"
export WINDOW_STRIDE="${WINDOW_STRIDE:-63}"   # 5 s with 50% overlap @ 25 Hz
...
export RAW_PARQUET="${RAW_PARQUET:-$(cd "${DIR}/../.." && pwd)/dataset/processed/pretrain_raw_windowed_w125.parquet}"
...
# ---------------------------------------------------------------------------
# Pretraining (shared across feature sets — uses raw 125-sample windows).
# ---------------------------------------------------------------------------
echo ">>> Stage 1: encoder pretraining on paper-windowed raw data (${WINDOW_LEN} samples)"
...
CNN_CKPT="${TS2VEC_CNN_DIR}/ts2vec_ep${PRETRAIN_EPOCHS}.pt"
ROB_CKPT="${TS2VEC_ROB_DIR}/ts2vec_ep${PRETRAIN_EPOCHS}.pt"
